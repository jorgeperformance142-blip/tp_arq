<form method="post" action="{{ $action }}" class="space-y-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
      ...
      <input type="number" name="min_puntos" min="0" value="{{ old('min_puntos', $nivel->min_puntos ?? '') }}" required
             class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-500 focus:ring-slate-500" />
    ...
    <div>
      ...
      <input type="number" name="max_puntos" min="0" value="{{ old('max_puntos', $nivel->max_puntos ?? '') }}"
             class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-500 focus:ring-slate-500" />
  ...
  <div>
    ...
    <textarea name="beneficios" rows="4" class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-500 focus:ring-slate-500">{{ old('beneficios', isset($nivel) ? implode("\n", $nivel->beneficios ?? []) : '') }}</textarea>
    @error('beneficios')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
  </div>

  <div class="flex items-center justify-end gap-3">
    <a href="{{ route('niveles.index') }}" class="px-4 py-2 rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50">Cancelar</a>
    <button type="submit" class="px-4 py-2 rounded-lg bg-slate-900 text-white hover:bg-slate-800">{{ $submit }}</button>
  </div>
</form>
